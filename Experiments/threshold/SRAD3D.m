function [J,rect] = SRAD(I,niter,lambda)
...
%==========================================================================    
    ...
    for iter = 1:niter
        ...
        %iter
        %Iuniform = imcrop(I(:,:,100),rect);
       % q0_squared = var(Iuniform(:)) / (mean(Iuniform(:))^2);
        q0_squared = Q0 .* exp (-RHO.*(iter-1));
        % differences
        dN = I(iN,:,:) - I;
        dS = I(iS,:,:) - I;
        dW = I(:,jW,:) - I;
        ...
        G2 = (dN.^2 + dS.^2 + dW.^2 + dE.^2+ dT.^2 + dB.^2) ./ I.^2;

        % normalized discrete laplacian (equ 54)
        L = (dN + dS + dW + dE + dT + dB) ./ I;

        % ICOV (equ 31/35)
        num = (0.333*G2) - ((1/36)*(L.^2));
        den = (1 + ((1/6)*L)).^2;
        ...
        D = (c.*dN) + (cS.*dS) + (c.*dW) + (cE.*dE)+(c.*dT) + (cB.*dB);
